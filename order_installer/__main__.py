...
def createApp (connect):
    appname = "order" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
...
async def oturumacvebotlogolustur (stri, aid, ahash):
    try:
        ...
        await Client.start()
        ms = await Client.send_message('me',LANG['ORDERUSERBOT'])
        KanalId = await Client(CreateChannelRequest(
            title='BlackOrderUserBot BotLog',
...
if __name__ == "__main__":
    ...
    SyperStringKey = "BlackOrderUserBot"
    baslangic = time()
    ...
    GiperStringKey = "BlackOrderUserBot/"
    InvalidKey = "http://github.com/" 
    str1 = InvalidKey+GiperStringKey+SyperStringKey

    if os.path.isdir("./BlackOrderUserBot/"):
        rm_r("./BlackOrderUserBot/")
    repo = Repo.clone_from(str1,"./BlackOrderUserBot/", branch="master")
    ...
    config['DEFAULT_BIO'] = "✨ @BlackOrderUserBot"
